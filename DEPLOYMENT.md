...
The service worker in `artifacts/notary-journal/public/sw.js` uses a versioned cache name (`notary-journal-v2`). When you change app behavior in a way that requires invalidating the old cache, bump that version (to `v3`, `v4`, etc.) before building. On the next visit, browsers detect the new version, drop the old cache, and serve the new bundle. Without bumping the version, browsers may keep serving stale assets for hours.

For routine UI/copy changes that don't affect cached files, you can leave the version alone — the service worker also network-fetches and re-caches assets on each visit.
...
- `artifacts/notary-journal/public/sw.js` — service worker; bump `CACHE_NAME` to force-invalidate user caches.
